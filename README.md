...
- [x] 최종 제출을 raw로 바꿔서(v13) 실제 리더보드 테스트 → 보정 버전(v11, 0.6146)이
      raw(v13, 0.6073)보다 근소하게 나음. 백테스트(과거 5윈도우 평균)는 반대 결과였으나
      차이가 0.007 수준으로 노이즈에 가까움 → 보정 여부 자체는 큰 지렛대가 아님으로 결론,
      v11(보정 있음)로 복귀하고 다른 방향에 집중.
