...
**FICR (Financial Cost Recovery Rate, per group):** `획득 정산금 / 이론상 최대 정산금`, averaged across the 3 groups. This mirrors Korea's actual KPX renewable-energy forecast-incentive settlement system, where the hourly bonus paid is a **step function of the hourly error rate** (better accuracy bands pay more per kWh, beyond some error threshold the bonus is zero). ⚠️ **The exact error-band → payout table was rendered as a chart/image on the evaluation page and did not extract as text via fetch** — this must be confirmed manually by opening the page in a browser (or screenshotting it) before FICR can be implemented exactly. Until confirmed, treat FICR as: monotonically non-increasing in `|pred−actual|/capacity`, roughly piecewise-linear/step, and **not identical to NMAE** (so optimizing NMAE alone is not guaranteed to maximize FICR — see 1.3).
