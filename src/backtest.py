...
v28 이전: 각 윈도우를 "그 구간만 빼고 나머지 전체로 학습 -> 그 구간에서 평가"하는
방식(blocked time-series cross-validation)을 썼음. 하지만 이 방식은 평가 윈도우보다
'미래'의 데이터도 학습에 포함시키는 문제가 있었음 - 실제 대회에서는 있을 수 없는
상황(diagnose_walkforward.py로 평균 +0.008~0.010 낙관 편향 확인, 다른 대회 경험을
정리한 외부 플레이북 문서에서 지적받고 발견함).
v28: 그 시점 '이전' 데이터만 사용하는 진짜 walk-forward로 교체. 이전 데이터가
너무 적은 초기 윈도우(2022년 봄)는 평가 자체가 불가능해 제외함(윈도우 5개->4개).
...
BACKTEST_WINDOWS = [
    # v28: diagnose_walkforward.py로 확인된 '미래 데이터 누수' 문제(blocked-CV) 수정.
    # 2022-04~06 윈도우는 그 이전 데이터가 3개월 미만이라 진짜 walk-forward로는
    # 평가가 불가능해 제외함 (기존엔 미래 데이터로 채워서 억지로 평가했었음 - 부정확).
    ("2022-10-01", "2022-12-30"),   # 가을~겨울 2022 (이전 데이터 약 9개월, 그룹1,2만)
    ("2023-04-01", "2023-06-30"),   # 봄 2023 (이전 데이터 약 15개월, 전체 그룹)
    ("2023-10-04", "2024-01-02"),   # 가을~겨울 2023 (이전 데이터 약 21개월, 전체 그룹)
    ("2024-04-01", "2024-06-30"),   # 봄 2024 (이전 데이터 약 27개월, 전체 그룹)
]
...
def evaluate_window(holdout_start: pd.Timestamp, holdout_end: pd.Timestamp,
    ...
    for target in TARGET_COLS:
        ...
        # v28 [버그수정]: 기존 '~is_holdout & ~is_calib'는 평가 윈도우보다 미래의 데이터도
        # 학습에 포함시켜(blocked-CV) 실제보다 낙관적인 점수를 만들었음
        # (diagnose_walkforward.py로 평균 +0.008~0.010 낙관 편향 확인).
        # -> 그 시점 이전 데이터만 사용하는 진짜 walk-forward로 교체.
