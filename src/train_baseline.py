...
from features import (
    ...
    load_clean_scada_group,
    fit_ws_calibration,
    apply_ws_calibration,
    fit_power_curve,
    apply_power_curve,
)
...
USE_LOG_TARGET = False    # v20에서 시도했으나 실제 리더보드에서 무효(0.6072)로 확인되어 끔
...
def build_group_weather(ldaps: pd.DataFrame, gfs: pd.DataFrame, group_coords: dict) -> dict:
    ...
    for group, coord in group_coords.items():
        ...
        # v22-1: 그룹별 격자 집계 차별화 (팀원 EDA 실증 근거 - eda_report.md Open Question 7):
        #   그룹1은 좌표가 격자 클러스터 안쪽 깊숙이 있어 16개 전체 평균이 우세(상관 0.730 vs 최근접 0.615),
        #   그룹2/3은 특정 격자에 붙어있어 좁은 범위(최근접 위주)가 우세(0.757/0.779).
        # -> 그룹1은 LDAPS 전체 16격자 평균, 그룹2/3은 기존대로 최근접 3개.
        #    calib으로 '고른' 게 아니라 팀원의 독립적 상관 분석에 근거한 확정적 결정.
        if group == "kpx_group_1":
            ldaps_grids = sorted(ldaps["grid_id"].unique().tolist())  # 전체 16개
        else:
        ...
        # v21-1 [버그수정]: 격자별 풍속을 '먼저' 계산한 뒤 집계 (벡터 상쇄 방지).
        # 기존 방식(u/v 성분을 먼저 평균 -> 풍속 계산)은 방향이 다른 바람이 상쇄되어
        # 풍속이 과소평가됨 (팀원 EDA에서 발견). max 집계도 함께 추가 (능선 지형 특성).
        ldaps_ws10_fixed = aggregate_wind_speed_per_grid(
            ldaps, ldaps_grids, "heightAboveGround_10_10u", "heightAboveGround_10_10v", "ldaps_ws10"
        )
        gfs_ws10_fixed = aggregate_wind_speed_per_grid(
            gfs, gfs_grids, "heightAboveGround_10_10u", "heightAboveGround_10_10v", "gfs_ws10"
        )
        gfs_ws100_fixed = aggregate_wind_speed_per_grid(
            gfs, gfs_grids, "heightAboveGround_100_100u", "heightAboveGround_100_100v", "gfs_ws100"
        )
        weather = weather.merge(ldaps_ws10_fixed, on="forecast_kst_dtm", how="left")
        weather = weather.merge(gfs_ws10_fixed, on="forecast_kst_dtm", how="left")
        weather = weather.merge(gfs_ws100_fixed, on="forecast_kst_dtm", how="left")

        # v21-2 [신규피처]: LDAPS와 GFS의 풍속 불일치. 팀원 EDA에서 상관 0.60으로
        # GFS 단독 풍속(0.585~0.601)만큼 강력한 신호로 실증됨. 두 예보 모델이 서로
        # 다르게 예측하는 정도 = 예보 불확실성의 대리지표. 지금까지 한 번도 안 씀.
        weather["ldaps_gfs_ws10_diff"] = weather["ldaps_ws10_gridfirst_mean"] - weather["gfs_ws10_gridfirst_mean"]
        weather["ldaps_gfs_ws10_diff_abs"] = weather["ldaps_gfs_ws10_diff"].abs()
...
def main():
    ...
    # v22-2: SCADA 정제 데이터 (팀원 EDA에서 실증된 .sum()/end-of-hour 컨벤션 반영판).
    # v16 물리 피처가 무효했던 원인이 .mean() 스케일 버그(값이 1/6로 축소)였을 가능성이
    # 있어 수정 후 재시도.
    ...
    scada_source = {"kpx_group_1": scada_vestas, "kpx_group_2": scada_vestas, "kpx_group_3": scada_unison}
    scada_clean = {}
    for g, (prefix, turbine_ids) in SCADA_GROUP_TURBINES.items():
        scada_clean[g] = load_clean_scada_group(scada_source[g], prefix, turbine_ids)
    ...
    for target in TARGET_COLS:
        ...
        # v22-2: SCADA 2단계 물리 피처 재시도 (.sum()/end-of-hour 수정판).
        # 예보풍속 -> (환산식) -> 추정 실측풍속 -> (SCADA 실측 파워커브) -> 물리 예상 발전량.
        # v16과 구조는 같지만 SCADA 집계 버그(.mean()으로 1/6 축소)가 수정된 상태.
        scada = scada_clean[target].rename(columns={"kst_dtm": "forecast_kst_dtm"})
        scada_merged = train_labels[["kst_dtm"]].rename(columns={"kst_dtm": "forecast_kst_dtm"}).merge(
            scada, on="forecast_kst_dtm", how="left"
        )[mask_label].reset_index(drop=True)
        scada_merged.index = X_all.index

        ws_calib = fit_ws_calibration(
            X_all.loc[is_train, power_curve_col], scada_merged.loc[is_train, "actual_ws"]
        )
        ws_fallback = X_all.loc[is_train, power_curve_col].median()
        est_ws_train = pd.Series(
            apply_ws_calibration(ws_calib, X_all.loc[is_train, power_curve_col], ws_fallback),
            index=X_all.loc[is_train].index,
        )
        physics_curve, physics_fallback = fit_power_curve(
            est_ws_train, scada_merged.loc[is_train, "actual_energy_kwh"].fillna(y_all[is_train])
        )
        est_ws_all = pd.Series(
            apply_ws_calibration(ws_calib, X_all[power_curve_col], ws_fallback), index=X_all.index
        )
        X_all["scada_physics_estimate"] = apply_power_curve(
            physics_curve, physics_fallback, est_ws_all, CAPACITY_KWH[target]
        )
        ...
        # v22-3: feature importance 진단 (다음 버전에서 저기여 피처 정리할 근거 확보용).
        # LightGBM 첫 시드 모델 기준. importance=0인 피처는 그 모델이 한 번도 안 쓴 피처.
        first_lgbm = trained[0][2]
        imp = pd.Series(first_lgbm.feature_importances_, index=X_tr_imp.columns).sort_values()
        zero_imp = imp[imp == 0]
        print(f"[{target}] 피처 {len(imp)}개 중 importance=0 (한 번도 안 쓰임): {len(zero_imp)}개")
        if len(zero_imp) > 0:
            print("   -> " + ", ".join(zero_imp.index.tolist()[:15]) + (" ..." if len(zero_imp) > 15 else ""))
        print(f"[{target}] 상위 10개 피처: " + ", ".join(imp.tail(10).index.tolist()[::-1]))
        ...
        est_ws_test = pd.Series(
            apply_ws_calibration(ws_calib, X_test[power_curve_col], ws_fallback), index=X_test.index
        )
        X_test["scada_physics_estimate"] = apply_power_curve(
            physics_curve, physics_fallback, est_ws_test, CAPACITY_KWH[target]
        )
